...
namespace UnityStandardAssets.Utility
{
	public class WaypointCircuit : MonoBehaviour
	{
		...
		public RoutePoint GetRoutePoint(float dist, out int lastWaypoint)
		{
			Vector3 routePosition = GetRoutePosition(dist, out lastWaypoint);
			Vector3 routePosition2 = GetRoutePosition(dist + 0.1f, out lastWaypoint);
			return new RoutePoint(routePosition, (routePosition2 - routePosition).normalized);
		}

		public Vector3 GetRoutePosition(float dist, out int lastWaypoint)
		{
			int i = 0;
			if (Length == 0f)
			{
		...
		private void DrawGizmos(bool selected)
		{
			...
			if (smoothRoute)
			{
				...
				{
					int lastWaypoint;
					Vector3 routePosition = GetRoutePosition(num + 1f, out lastWaypoint);
